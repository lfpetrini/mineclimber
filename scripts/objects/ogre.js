...
function Ogre() {
	...
	this.animation = new Animation(0, ANIMATION_SET_REPO.get("ogre_standing"));
	this.state = "standing";
	this.falling = true;
}
...
Ogre.prototype.update = function(gameTime) {
				...
				if(this.state != "attacking") {
					if(this.x - this.map.player.x + this.map.player.width <= 50) {
						if(gameTime - this.lastAttack >= this.attackInterval) {
							...
							this.animation = new Animation(gameTime, ANIMATION_SET_REPO.get("ogre_attacking"));
							var tempAttackBox = new EnemyAttackBox(gameTime, this, gameTime + 300);
							tempAttackBox.x = this.lookingLeft ? this.x - 18 : this.x + 9;
							tempAttackBox.y = this.y;
							tempAttackBox.height = 40;
			...
			if(this.lastY == this.y) {
				if(this.lastX != this.x) {
					if(this.state != "running" && this.state != "attacking") {
						this.animation = new Animation(gameTime, ANIMATION_SET_REPO.get("ogre_running"));
						this.state = "running";
					...
					if(this.state != "standing") {
						this.animation = new Animation(gameTime, ANIMATION_SET_REPO.get("ogre_standing"));
						this.state = "standing";
						this.stateStart = gameTime;
					}
				}
...
Ogre.prototype.setMaxHealth = function(health) {
	this.maxHealth = health;
	this.uiPixelPerHealth = this.uiTotalHealthBarWidth / health;
}

Ogre.prototype.setHealth = function(health) {
...
Ogre.prototype.collide = function(object, gameTime) {
	//if(this.dead)
	//	return;
}

Ogre.prototype.tileCollide = function(direction, tileType, gameTime) {
	if(tileType == 3) {
		for(var i = 0; i < 7; i++) {
